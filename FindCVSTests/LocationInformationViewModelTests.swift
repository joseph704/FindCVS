...
class LocationInfomationViewModelTests: XCTestCase {
    ...
    func testSetMapCenter() {
        ...
        let currenLocationButtonTapped = PublishSubject<Void>()

        currentLocationButtonTapEvent
            .subscribe(currenLocationButtonTapped)
            .disposed(by: disposeBag)

        let moveToCurrentLocation = currenLocationButtonTapped
